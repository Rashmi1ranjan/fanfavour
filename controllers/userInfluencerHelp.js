...
router.post('/save', protectRouteWithRole([SUPER_ADMIN]), async (req, res) => {
    ...
    if (_id === '') {
        ...
        return successResponse(res, {}, 'user influencer help Save successfully', 200)
    } else {
        const newValues = {
            title: title,
            popup_intro: popupIntro,
...
router.post('/get/popup-with-notification-count', async (req, res) => {
    ...
    try {
        ...
        const unreadCount = await getUserUnreadNotificationCount(userRegisteredDate, currentDateAndTime, domain, userId, websiteTagIds)
 ...
 */
async function getUserUnreadNotificationCount(userRegisteredDate, currentDateAndTime, domain, userId, tagIds) {
...
router.post('/influence-help-read-count', protectRouteWithRole([SUPER_ADMIN]), async (req, res) => {
    ...
    try {
        const readCount = await UserHelpReadCount.countDocuments({ help_id: id })
        return successResponse(res, readCount, 'get influencer help read count successfully', 200)
    } catch (error) {
        return errorResponse(res, error, error.message, 500)
    }
})
